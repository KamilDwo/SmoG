    ...
    render() {
        ...
        if(pointData){
            ...
            if(sensorIndex >= 0 || pointValues){
                ...
                box3 = <Card size="small">   
                    <Tabs defaultActiveKey="1" animated={false}>
                        <TabPane tab="PM10" key="1"><b>{Math.round(preParam2.values[0].value)}</b> µg/m³</TabPane>
                        <TabPane tab="PM2.5" key="2"><b>{Math.round(preParam2.values[1].value)}</b> µg/m³</TabPane>
                        <TabPane tab="PM1" key="3"><b>{Math.round(preParam2.values[2].value)}</b> µg/m³</TabPane>
                        <TabPane tab="Pressure" key="4"><b>{Math.round(preParam2.values[3].value)}</b> hPa</TabPane>
                    </Tabs>                 
                </Card>  
                box4 = <Card size="small">  
                    <Row>
                        <Col span={12} style={{ textAlign: 'center' }}>
                            <span className="chart-title">Humidity</span>
                            <Progress type="circle" percent={Math.round(preParam2.values[4].value)} />                      
                        </Col>
                        <Col span={12} style={{ textAlign: 'center' }}>
                            <span className="chart-title">Temperature</span>  
                            <b style={{ fontSize: '25px' }}>{Math.round(preParam2.values[5].value)}</b> °C    
                        </Col>
                    </Row>
                </Card>
            } 
